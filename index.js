...
program
	...
	.option('-r, --routes <routes>', 'List of lowest hierarchal position routes (EX: "/users/:user_id/posts/:post_id/comments"). Use comma to separate (no spaces).', list)
	.option('-a, --authentication-link <authlink>', 'Link to be used for authentication purposes')
	.parse(process.argv);

co(function *() {
	var mongooseConnection = (!program.mongooseConnection ? yield prompt('Mongoose Connection link: ') : program.mongooseConnection),
		routes = (!program.routes ? list(yield prompt('Routes (EX: "/users/:user_id/posts/:post_id/comments"), separate by commas: ')) : program.routes),
		authenticationLink = (!program.authenticationLink ? yield prompt('Link to be used for authentication purposes (EX: "/users"): ') : program.authenticationLink);
